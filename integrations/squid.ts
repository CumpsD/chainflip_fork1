...
const squidLogMap: Partial<
  Record<SwapStatus | 'default', { order: number; message: string }>
> = {
  default: {
    order: 0,
...
const mapSquidChainId = (
  squidChainType: SquidSdkChainType,
  squidChainId: string
): ChainId =>
...
const mapSquidTokenAddress = (squidAddress: string): string =>
  squidAddress === SQUID_NATIVE_TOKEN_ADDRESS
...
const mapSquidAction = (
  squidAction: SquidSdkRouteAction
): RouteResponseStep | undefined => {
...
const mapSquidStatus = (
  status: SquidSdkStatusResponse | undefined
): SwapStatus => {
...
export class SquidIntegration implements BaseIntegration {
  ...
  getChains = async () => {
    ...
    return chains
      .map((chain) =>
        chainById(mapSquidChainId(chain.chainType, chain.chainId))
      )
      .filter(isTruthy)
  }
      ...
      if (route.transactionHash) {
        if (!statusOrder || item.order <= statusOrder) {
          ...
          eventLogs.unshift({
            ...
            status:
              item.order === statusOrder && item.order < 3
                ? 'processing'
                : 'success',
          })
        }
