...
export const getDeterministicRouteId = (route: Omit<RouteResponse, 'id'>) => {
  ...
  return ethers.keccak256(
    ethers.toUtf8Bytes(JSON.stringify(identificationData))
  )
}
...
export class IntegrationManager {
  static from(
    integrations: Record<
      Integration,
      BaseIntegration | Promise<BaseIntegration>
    >
  ) {
  ...
  private constructor(
    private readonly integrations: Record<Integration, BaseIntegration>
  ) {
  ...
  getChains = (): Observable<Chain[]> =>
    from(this.ensureReady()).pipe(
      mergeMap(() =>
        Object.values(this.integrations).map((i) => i.getChains())
      ),
  ...
  getDestinationChains = (srcChainId: ChainId): Observable<Chain[]> =>
    from(this.ensureReady()).pipe(
      mergeMap(() =>
        Object.values(this.integrations).map((i) =>
          i.getDestinationChains(srcChainId)
        )
      ),
      mergeMap((v) => v.catch((err) => err)),
      scan(
  ...
  getRoutes = async (routeRequest: RouteRequest): Promise<RouteResponse[]> => {
    ...
    const promises = Object.values(this.integrations).map((i) =>
      i.getRoutes(routeRequest)
    )
