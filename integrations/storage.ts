...
export const storeRouteInLocalStorage = (
  ...
  localStorage.setItem(
    routeStorageKey(integration, swapId),
...
export const loadRouteFromLocalStorage = (
  ...
  const stringifiedRoute = localStorage.getItem(
    routeStorageKey(integration, swapId)
  )
  ...
  return stringifiedRoute
    ? (JSON.parse(
        stringifiedRoute,
        stringifiedTokenAmountReviver
      ) as StoredRouteResponse)
    : undefined
}
...
export const storeTransactionHashInLocalStorage = (
  ...
  if (!storedRoute)
    throw new Error('route not found when storing transaction hash')
...
export const storeDepositChannelIdInLocalStorage = (
  ...
  const storedRoute = loadRouteFromLocalStorage(integration, swapId)
  if (!storedRoute)
    throw new Error('route not found when storing deposit channel id')
...
export const getDepositModeFromLocalStorage = () => {
  const result = depositModeSchema.safeParse(
    localStorage.getItem('depositMode')
  )
...
export const storeDepositModeInLocalStorage = (
  value: 'channel' | 'contract'
) => {
  if (!value) {
