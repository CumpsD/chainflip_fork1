...
// import { CHAINFLIP_PREFIX } from '../consts'
...
export function objectStringToValue(
  nestedObject: NestedRecord,
  stringKey: string
): NestedRecord {
  return stringKey
    .split('.')
    .reduce(
      (acc, key) => (typeof acc === 'object' ? acc?.[key] : acc),
      nestedObject
    )
}
...
export type Comparator = Record<
  'ASC' | 'DESC',
  (a: ComparatorArg, b: ComparatorArg) => 1 | 0 | -1
>
...
export const COMPARATORS: Comparator = {
  ...
  DESC: (a: ComparatorArg, b: ComparatorArg) =>
    (compareAscending(a, b) * -1) as -1 | 0 | 1,
}
...
export const initSentry = () => {
  ...
  try {
    Sentry.init({
      ...
      tracesSampleRate:
        Number(process.env.NEXT_PUBLIC_SENTRY_SAMPLE_RATE) || undefined,
  ...
  if (typeof obj === 'object' && obj !== null) {
    return Object.fromEntries(
      Object.entries(obj).map(([key, value]) => {
        ...
        return [
          key,
          replacer ? replacer(value) : deepReplace(value, replacements),
        ]
      })
    )
  }
...
// export const validatorTotalBalance = (
//   validator: CastCacheValidatorFragment
// ): TokenAmount => validator.lockedBalance.add(validator.unlockedBalance)
